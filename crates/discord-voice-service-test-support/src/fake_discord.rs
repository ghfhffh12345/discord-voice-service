...
enum FakeDiscordControl {
    ...
    InjectLateDaveListenerTransition {
        ...
        commit_before_prepare_epoch: bool,
...
impl FakeDiscordPeer {
    ...
    async fn spawn_with_options_and_ready_delay(
        ...
        let dave_invalid_commit_welcome_ids_state = Arc::clone(&dave_invalid_commit_welcome_ids);
        ...
        tokio::spawn(async move {
            if dave_scenario == DaveScenario::Disabled
                ...
                && pre_session_description_events.is_empty()
            {
                loop {
                    let Ok((stream, _)) = listener.accept().await else {
                        return;
                    };
                    let gateway_path_accept = Arc::clone(&gateway_path_state);
                    let mut ws =
                        accept_hdr_async(stream, move |request: &Request, response: Response| {
                            *gateway_path_accept.lock().unwrap() = Some(request.uri().to_string());
                            Ok(response)
                        })
                        .await
                        .unwrap();
                    ...
                    loop {
                        let message = tokio::select! {
                            outbound = gateway_messages_rx.recv() => {
                        ...
                        match payload.get("op").and_then(Value::as_u64) {
                            Some(0) => {
                                ...
                                *identified_user_id_state.lock().await =
                                    required_fields_present.then_some(user_id);
            ...
            let mut pending_late_commit_before_prepare_epoch = false;
            ...
            loop {
                        ...
                        match control {
                            ...
                            FakeDiscordControl::InjectLateDaveListenerTransition {
                                ...
                                let result = async {
                                    ...
                                    .await
                                    .map_err(|_| VoiceError::InvalidState("fake gateway send failed"))?;
                                    if commit_before_prepare_epoch {
                                        pending_late_commit_before_prepare_epoch = true;
                                    } else {
                                        ws.send(Message::Text(
                                            json!({
                                                "op": 24,
                            ...
                            FakeDiscordControl::InjectStaleDaveProposal {
                                late_user_id,
                                response,
                            } => {
                                let result = async {
                                    let external_sender = dave_external_sender.as_ref().ok_or(
                                        VoiceError::InvalidState(
                                            "fake dave external sender unavailable",
                                        ),
                                    )?;
                                    let external_sender_bytes = external_sender
                                        .marshalled_external_sender()
                                        .map_err(|_| {
                                            VoiceError::InvalidState(
                                                "fake dave external sender marshal failed",
                                            )
                                        })?;
                                    let runtime_user_id = identified_user_id_state
                                        .lock()
                                        ...
                                        .ok_or(VoiceError::InvalidState(
                                            "fake runtime user id unavailable",
                                        ))?;
                                    let group_id = group_id_from_user_context(
                                        &runtime_user_id,
                                        &dave_group_id_state,
                                    );
                                    let mut late_member = DaveSession::new(None).map_err(|_| {
                                        VoiceError::InvalidState(
                                            "fake stale dave session create failed",
                                        )
                                    })?;
                                    late_member
                                        .set_external_sender(&external_sender_bytes)
                                        .map_err(|_| {
                                            VoiceError::InvalidState(
                                                "fake stale dave external sender invalid",
                                            )
                                        })?;
                                    late_member
                                        ...
                                        .map_err(|_| {
                                            VoiceError::InvalidState(
                                                "fake stale dave session init failed",
                                            )
                                        })?;
                                    let late_key_package = late_member.key_package().map_err(
                                        |_| {
                                            VoiceError::InvalidState(
                                                "fake stale dave key package failed",
                                            )
                                        },
                                    )?;
                                    let proposal = external_sender
                                        .propose_add(99, &late_key_package)
                                        .map_err(|_| {
                                            VoiceError::InvalidState(
                                                "fake stale dave proposal failed",
                                            )
                                        })?;
                                    ws.send(Message::Text(
                                        json!({
                                    ...
                                    .await
                                    .map_err(|_| {
                                        VoiceError::InvalidState("fake gateway send failed")
                                    })?;
                            ...
                            FakeDiscordControl::ReplayEstablishedJoinCommitTransition {
                                response,
                            } => {
                                let result = async {
                                    let commit = established_join_commit_state
                                        .lock()
                                        .unwrap()
                                        .clone()
                                        .ok_or(VoiceError::InvalidState(
                                            "fake established join commit unavailable",
                                        ))?;
                                    ws.send(Message::Binary(Bytes::from(
                                        dave_prepare_commit_transition_message(
                                            12,
                ...
                if let Message::Text(text) = message {
                    ...
                    match payload.get("op").and_then(Value::as_u64) {
                        ...
                        Some(31) if dave_scenario != DaveScenario::Disabled => {
                            if let Some(transition_id) = payload
                                .get("d")
                                .and_then(|data| data.get("transition_id"))
                                .and_then(Value::as_u64)
                                .and_then(|value| u16::try_from(value).ok())
                            {
                    ...
                    match bytes.first().copied() {
                            ...
                            *dave_key_package_count_state.lock().await += 1;
                                ...
                                && !*saw_dave_prepare_epoch_state.lock().await
                            {
                                ...
                                *saw_dave_prepare_epoch_state.lock().await = true;
                                ...
                                && !key_package_before_external_sender
                            {
                                *saw_dave_key_package_after_external_sender_state.lock().await =
                                    true;
                            }
                            ...
                            if dave_scenario == DaveScenario::PrepareBackedWelcomeWithStrayFollowUp {
                                ...
                                queued_stray_welcome = Some(dave_welcome_message(
                                    ...
                                    external_sender,
                                    DaveWelcomeMessage {
                                        scenario: DaveScenario::NewGroup,
                                ...
                                == DaveScenario::EstablishedGroupJoinWithProposalsDelayedUntilControl
                            {
                                let welcome_message = dave_welcome_message(
                                    ...
                                    external_sender,
                                    DaveWelcomeMessage {
                                        scenario: dave_scenario,
                                        runtime_user_id: &user_id,
                                        transition_id: DAVE_TRANSITION_ID,
                                        key_package: &bytes[1..],
                                        cached_commit: Some(&established_join_commit_state),
                                    },
                                );
                                ...
                                && pending_late_member.is_some() =>
                        {
                            *saw_dave_commit_welcome_state.lock().await = true;
                            ...
                            .await
                            .unwrap();
                            if pending_late_commit_before_prepare_epoch {
                                ws.send(Message::Text(
                                    json!({
                                        "op": 24,
                                        "seq": 13,
                                ...
                                .await
                                .unwrap();
                                pending_late_commit_before_prepare_epoch = false;
                            }
                        }
                        _ => {}
                    }
    ...
    pub async fn send_speaking_state_without_user(
        &self,
        speaking: u64,
        ssrc: u32,
    ) -> Result<(), VoiceError> {
    ...
    pub async fn send_client_disconnect(&self, user_id: &str) -> Result<(), VoiceError> {
        self.gateway_messages
            .send(Message::Text(
                json!({
                    "op": 13,
    ...
    pub async fn inject_late_dave_listener_transition(
        ...
        self.control_messages
            .send(FakeDiscordControl::InjectLateDaveListenerTransition {
                ...
                commit_before_prepare_epoch: false,
                response: response_tx,
            })
            .map_err(|_| VoiceError::InvalidState("fake dave control send failed"))?;
        response_rx
    ...
    pub async fn inject_late_dave_listener_commit_before_prepare_epoch(
        &self,
        late_user_id: &str,
    ) -> Result<(), VoiceError> {
        let (response_tx, response_rx) = oneshot::channel();
        self.control_messages
            .send(FakeDiscordControl::InjectLateDaveListenerTransition {
                late_user_id: late_user_id.to_owned(),
                prelude_speaking_events: 0,
                commit_before_prepare_epoch: true,
                response: response_tx,
            })
            .map_err(|_| VoiceError::InvalidState("fake dave control send failed"))?;
        response_rx
    ...
    pub async fn inject_stale_dave_proposal(&self, late_user_id: &str) -> Result<(), VoiceError> {
        let (response_tx, response_rx) = oneshot::channel();
        self.control_messages
            .send(FakeDiscordControl::InjectStaleDaveProposal {
                late_user_id: late_user_id.to_owned(),
                response: response_tx,
            })
            .map_err(|_| VoiceError::InvalidState("fake dave control send failed"))?;
    ...
    pub async fn replay_established_join_commit_transition(&self) -> Result<(), VoiceError> {
        let (response_tx, response_rx) = oneshot::channel();
        self.control_messages
    ...
    pub async fn discovery_count_at_least(&self, minimum: usize) -> usize {
        wait_for_value(&self.discovery_count, |count| *count >= minimum).await
    }
    ...
    pub async fn speaking_state_count_at_least(&self, speaking: u64, minimum: usize) -> usize {
    ...
    pub async fn speaking_state_count(&self, speaking: u64) -> usize {
    ...
    pub async fn saw_dave_key_package_count_at_least(
        &self,
        minimum: usize,
        timeout: Duration,
    ) -> bool {
        wait_for_value_with_timeout(&self.dave_key_package_count, timeout, |count| {
            *count >= minimum
        })
        .await
            >= minimum
    ...
    pub async fn saw_dave_invalid_commit_welcome_within(
        &self,
        transition_id: u16,
        timeout: Duration,
    ) -> bool {
        wait_for_value_with_timeout(&self.dave_invalid_commit_welcome_ids, timeout, |ids| {
...
struct DaveWelcomeMessage<'a> {
    scenario: DaveScenario,
    runtime_user_id: &'a str,
    transition_id: u16,
    key_package: &'a [u8],
    cached_commit: Option<&'a Arc<StdMutex<Option<Vec<u8>>>>>,
}
...
fn dave_welcome_message(
    ...
    message: DaveWelcomeMessage<'_>,
) -> Vec<u8> {
    let DaveWelcomeMessage {
        scenario: dave_scenario,
        runtime_user_id,
        transition_id,
        key_package,
    ...
    let proposal_epoch = match dave_scenario {
        ...
        DaveScenario::EstablishedGroupJoin
        | DaveScenario::EstablishedGroupJoinDelayedUntilControl
        | DaveScenario::EstablishedGroupJoinWithProposalsDelayedUntilControl => creator
            .epoch()
    ...
    if let Some(cached_commit) = cached_commit {
        *cached_commit.lock().unwrap() = Some(commit.clone());
    }
    creator
        .process_commit(&commit)
