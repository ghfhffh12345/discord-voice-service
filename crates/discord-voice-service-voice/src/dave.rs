...
impl DaveRuntimeContext {
    pub fn from_session(
        ...
        let encrypt_ratchet = session
            ...
            .map_err(|_| VoiceError::InvalidState("voice dave self ratchet missing"))?;

        let mut encryptor = DaveEncryptor::new()
            .map_err(|_| VoiceError::InvalidState("voice dave encryptor create failed"))?;
        encryptor.assign_opus_ssrc(ssrc);
        encryptor
            .set_key_ratchet(&encrypt_ratchet)
            .map_err(|_| VoiceError::InvalidState("voice dave encryptor setup failed"))?;
