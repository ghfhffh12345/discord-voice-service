...
pub async fn connect(
    url: &str,
) -> Result<(VoiceWebSocketWriter, VoiceWebSocketReader), VoiceError> {
    let normalized_url = normalize_gateway_url(url)?;
    let (ws, _) = connect_async(normalized_url).await?;
    Ok(ws.split())
}

pub async fn send_json(ws: &mut VoiceWebSocketWriter, payload: Value) -> Result<(), VoiceError> {
    ws.send(Message::Text(payload.to_string().into())).await?;
    Ok(())
}

pub async fn send_binary(
    ws: &mut VoiceWebSocketWriter,
    payload: Vec<u8>,
) -> Result<(), VoiceError> {
...
fn normalize_gateway_url(url: &str) -> Result<String, VoiceError> {
    let uri: Uri = url.parse()?;
    let mut parts = uri.into_parts();
    if parts.scheme.is_none() || parts.authority.is_none() {
        return Err(VoiceError::InvalidState(
            "voice gateway url must be absolute",
        ));
    }
    ...
    Ok(Uri::from_parts(parts)
        .map_err(|_| VoiceError::InvalidState("voice gateway url could not be normalized"))?
        .to_string())
}
